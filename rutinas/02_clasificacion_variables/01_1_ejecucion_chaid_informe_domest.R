...
auxiliar <- pobreza %>% 
    group_by(domest, ndomest) %>% 
    summarise() %>% 
  filter(!domest %in% upm_domest$domest[upm_domest$n_estratos == 1])


# chaid
for (i in 1:(dim(auxiliar)[1])){
  
  nombre_domest <- auxiliar$ndomest[i]
  domestrato <- auxiliar$domest[i]
  
  pobreza_01 <- pobreza %>% 
    filter(domest == domestrato)
  
  print(i)
  rmarkdown::render(input = "rutinas/02_clasificacion_variables/55_chaid_informe.Rmd",
                    output_format = "html_document",
                    output_file = paste0(domestrato,"_chaid"),
                    output_dir = "intermedios/02_clasificacion_variables/html_domest/",
                    clean = T,
