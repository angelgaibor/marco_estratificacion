...
viv<-import("insumos/01_tratamiento/censo/viv_2022.csv")

### Vivienda

viv_ocu_pp <- viv %>% rename_all(tolower) %>%
  mutate(provincia = str_pad(i01, 2, "left", pad = "0"),
         ...
         colectiva = ifelse(v01 %in% c(9:18), 1, 0),
         sin_vivienda = ifelse(v01 %in% c(19), 1, 0),
         sin_viv = ifelse(is.na(v0201) & is.na(v0202), 1, 0),
         ocu_per_pre = ifelse(v0201 %in% c(1), 1, 0),
         ocu_per_aus = ifelse(v0201 %in% c(2:5), 1, 0)) 

# Formacion del marco de viviendas (viviendas ocupadas) y agregamos las variables
# id_upm y area
...
# Guardar las bases
export(marco_viv, "productos/01_tratamiento/marco_viv.rds")
